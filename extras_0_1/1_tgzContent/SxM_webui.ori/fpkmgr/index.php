...
$RegServer="http://highlevelbits.fr";
...
if ($ACTION == "REGISTER-NOW") {
 ...
 $ShellExecResult=shell_exec("wget $RegServer/download-MBWEW/Server -O /tmp/RegServer >/dev/null 2>&1");
  $RegServer=shell_exec("cat /tmp/RegServer");
 
       
  $ShellExecResult=shell_exec("rm -f /tmp/register.html");
  
  $ShellExecResult=shell_exec("wget $RegServer/download-MBWEW/MBWhiteRegister.php?MBWhiteRegister=true\&userid=$RegisterUser\&passwd=$RegisterPwd -O /tmp/tmpregister >/dev/null 2>&1");
...
if ($ACTION == "REGISTER-DISPLAY") {
  ...
  $ShellExecResult=shell_exec("wget $RegServer/download-MBWEW/MBWhiteRegister.html -O /tmp/register.html 2>&1");
...
#  }
  ...
  $tmp=shell_exec("wget  $RegServer/download-MBWEW/FP_list -O /proto/SxM_webui/fpkmgr/FP_list >/dev/nul 2>&1");
