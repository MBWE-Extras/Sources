...
Follow "40" "Check EXTRAS installed"
...
if [ ! -e /opt/bin/curl ] ;then
   Follow "50" "Installing OPTWARE libcurl"
   /opt/bin/ipkg update
   /opt/bin/ipkg install libcurl
fi

Follow "60" "Create folder package \"$PACK_NAME\""
if [ ! -d /proto/SxM_webui/extras/packs/${PACK_NAME} ] ;  then
   mkdir /proto/SxM_webui/extras/packs/${PACK_NAME}
fi

Follow "70" "Copy useful package \"$PACK_NAME\" files"
