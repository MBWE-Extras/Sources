...
Ext.BLANK_IMAGE_URL = '<?php echo _EXT_URL ?>/scripts/extjs3/resources/images/default/s.gif';
	...
	/**
	* This function is for remove HTML scruture inside response 
	* before Ext.decode analysis. *** MBWE adaptation ***
	*/
    function __RemoveHTML( oResponse ) {
//alert( oResponse.responseText );
//console.log( oResponse.responseText );
	var n = oResponse.responseText.search("</body>");
	if (n!=-1) { oResponse.responseText = oResponse.responseText.substring( 0, n ); }
	var n = oResponse.responseText.search("<body>") + 6;
	if (n!=-1) { oResponse.responseText = oResponse.responseText.substring( n ); }
	var n = oResponse.responseText.search("<script ");
	if (n!=-1) { oResponse.responseText = oResponse.responseText.substring( 0, n ); }
//alert( oResponse.responseText );
//console.log( oResponse.responseText );
    }
